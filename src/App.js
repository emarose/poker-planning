...
export default function PokerPlanningApp() {
  ...
  const handleDeleteStory = async (index) => {
    const updatedHistory = sessionHistory.filter((_, idx) => idx !== index);
    await setDoc(doc(db, 'sessions', SESSION_ID), {
  ...
  const handleClearAllStories = async () => {
    if (window.confirm('¿Estás seguro de que quieres eliminar todas las historias guardadas?')) {
      await setDoc(doc(db, 'sessions', SESSION_ID), {
        sessionHistory: []
      }, { merge: true });
      setSessionHistory([]);
        ...
        <div className="fixed left-4 top-24 z-30 bg-white/95 border border-secondary-orange rounded-lg shadow-md p-3 w-64 max-h-72 overflow-auto">
          ...
          <div className="space-y-2 mb-3">
            {sessionHistory.map((item, idx) => (
              <div key={`${item.storyTitle}-mini-${idx}`} className="border border-orange-200 rounded-md p-2 bg-white relative group">
                <button
                  onClick={() => handleDeleteStory(idx)}
                  className="absolute top-1 right-1 opacity-0 group-hover:opacity-100 transition-opacity text-red-500 hover:text-red-700 hover:bg-red-50 rounded p-1"
                  title="Eliminar historia"
                >
                  🗑️
                </button>
                <div className="text-sm font-semibold text-gray-900 truncate pr-6" title={item.storyTitle}>{item.storyTitle}</div>
          ...
          <button
            ...
            className="w-full text-xs font-semibold px-3 py-2 rounded-md bg-red-500 text-white hover:bg-red-600 transition-colors"
          >
            Limpiar todas
          </button>
        </div>
